...
4. JSON Serialization: by bundling all the log conents in one structured record with field names matching the one in the JSON.
...
5. Missing and invalid information will be serialized as null.

### Reasoning
Analyzing the app.log file, even though there are some inconsistencies, overall the following holds true:
1. Each seperate log entry had a timestamp, any lines that didn't have one were either part of another entry or just noise.
...
5. Some entries had missing or invalid parameters
